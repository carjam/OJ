...
export interface TrackRecommendation extends Track {
  distance: number;
}

export interface TrackRecord extends Track {
...
export const getSimilarTracks = async (trackId: string): Promise<TrackRecommendation[]> => {
  const data = await loadTracksData();
  return data[trackId]?.recommendations || [];
};
